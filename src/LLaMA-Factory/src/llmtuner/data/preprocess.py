...
"""
    ...
    def preprocess_pairwise_dataset(examples: Dict[str, List[Any]]) -> Dict[str, List[List[int]]]:
        ...
        model_inputs = {"prompt_ids": [], "chosen_ids": [], "rejected_ids": [], "criterion_ids": []}
        max_source_len = 2048
        max_target_len = 64
        ...
        for example in examples['conversations']:
            ...
            history = [_['value'] for _ in history]
            pos = example['pos']
            negs = example['negs']
            neg = negs[-1]

            inputs = format_llama(profile, history)
            prompt_ids = tokenizer.encode(inputs, add_special_tokens=False)

            chosen_ids = tokenizer.encode(pos, add_special_tokens=False)
            chosen_ids += [tokenizer.eos_token_id]
            rejected_ids = tokenizer.encode(neg, add_special_tokens=False)
            rejected_ids += [tokenizer.eos_token_id]

            if 'criterion' in example:
                criterion = example['criterion']
                criterion_ids = tokenizer.encode(criterion, add_special_tokens=False)
                criterion_ids += [tokenizer.eos_token_id]
        
            source_len, target_len = len(prompt_ids), max(len(chosen_ids), len(rejected_ids))
            if 'criterion' in example:
                target_len = max(target_len, len(criterion_ids))
            if source_len > max_source_len:
            ...
            if target_len > max_target_len:
                ...
                if 'criterion' in example:
                    criterion_ids = criterion_ids[:max_target_len]
            ...
            if 'criterion' in example:
                model_inputs["criterion_ids"].append(criterion_ids)

        if len(model_inputs['criterion_ids']) == 0:
            model_inputs.pop('criterion_ids')
        return model_inputs
    ...
    def print_pairwise_dataset_example(example: Dict[str, List[int]]) -> None:
        ...
        if 'criterion_ids' in example:
            print("criterion_ids:\n{}".format(example["criterion_ids"]))
            print("criterion:\n{}".format(tokenizer.decode(example["criterion_ids"], skip_special_tokens=False)))
